...
export default function PostDetailsPage() {
  ...
  const addComment = (e) => {
    ...
    setComments((prev) => [
      ...prev,
      { id: Date.now(), name: n, text: t, dateTime },
    ]);
    setText("");
    setName("");
  };
    ...
    <section className="flex flex-col md:flex-row justify-between gap-x-6 gap-y-5 mx-4 md:mx-12 my-8">
      <div className="w-full md:w-[68%]">
        <div className="mb-6">
          <Link
            href="/"
            className="px-4 py-2 rounded bg-gradient-to-r from-[#FE4F70] to-[#FFA387] text-white"
          >
            ← Back
          </Link>
        </div>
        ...
        <div className="mt-8 p-6">
          ...
          <ul className="mt-6 space-y-3">
            ...
            {comments.map((c) => (
              <li key={c.id} className="flex items-start gap-3 border-b pb-3">
                {/* Avatar */}
                <div className="w-10 h-10 rounded-full overflow-hidden flex-shrink-0">
                  <Image
                    src="/blog-img.jpg" // <-- you can place a default avatar in your public folder
                    alt={c.name}
                    width={40}
                    height={40}
